"""Regression checks for the narrow elementary5/6 extension boundary."""
import copy
import json
import unittest

from audit_branch_elementary_pages import exact_child_panel, restore_existing_html, scoped_grades, reviewed_school_lists, url, WRITING_NOTES


class ElementaryAuditTests(unittest.TestCase):
    def fixture(self):
        parent = '/지점안내/서울/시험점/시험동영어학원/'
        old_children = [{'grade': g, 'path': parent + g + '/', 'title': '시험동 ' + g + ' 영어학원'}
                        for g in ('중1', '중2', '중3', '고1', '고2')]
        ...
        nodes = [{'@type': 'WebPage', '@id': url(parent) + '#webpage',
                 ...
                 {'@type': 'ItemList', '@id': url(parent) + '#child-pages', 'numberOfItems': 5,
    ...
    def test_only_exact_two_elementary_buttons_reverse(self):
        before, after, children = self.fixture()
        self.assertTrue(restore_existing_html(before, after, 'neighborhood', children))

    def test_old_five_grade_links_schema_and_other_copy_cannot_change(self):
        before, after, children = self.fixture()
    ...
    def test_elementary_scope_and_school_level_not_inferred_from_other_grades(self):
        center = {'subjects': {'영어': {'grades': ['고1', '고2']}}, 'availableSubjects': ['영어']}
        self.assertNotIn('초5', scoped_grades(center, {}, '영어'))
        center['subjects']['영어']['grades'].append('초6')
        self.assertIn('초6', scoped_grades(center, {}, '영어'))
        self.assertNotIn('초5', scoped_grades(center, {}, '영어'))
        ...
        self.assertEqual(reviewed_school_lists('안내된 학교 정보에는 시험초, 시험중, 시험고가 제시되어 있습니다.', '초6'),
                         '안내된 학교 정보에는 시험초가 제시되어 있습니다.')
        history = '안내된 학교에는 시험초, 시험중, 시험고가 있고 고등 진학을 준비합니다.'
        self.assertEqual(reviewed_school_lists(history, '초6'), history)
