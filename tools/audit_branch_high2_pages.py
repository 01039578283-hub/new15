"""Independent high-school-year-2 source, navigation and preservation audit.
...
BASELINE = Path('C:/Users/1992k/Desktop/CodexData/tmp/site15-high2-2026-09-10/before.json.gz')
DOMAIN = 'https://xn--9p4bn5e3wjn0a.com'
ARCHIVE_SHAS = {'영어': '0ab40c333b9037422db6a199381362ca2a1fd7ebc46ada18083217d04efeea58',
                '수학': 'd495819df90b46e1cfd1f00e99329217b7bcd2de6b4488468d087e450783fdea'}
POLISHER_SHA256 = '6cf96be3eef5d91e97110468a2e3aeba8f443aeec90dc50c4d17b2c0a1597d2c'
GRADE_EDITOR_SHA256 = '78f80b9950d4d6f6de685609c45a2cd8be80b9f3a8b85a0077e4e7d3b1160ef5'
...
def complete_sentence_selection(before, after):
    ...
    for i, sentence in enumerate(old):
        if i in selected:
            continue
        if re.search(r'\d|[A-Za-z]|[×÷=]|[‘“「]|다만|단정|보장|개설|운영 여부', sentence.replace('고2', '')):
            return False
...
def reviewed_school_lists(value):
    ...
    ignored = {'참고', '그리고', '말고', '않고', '보고', '집중', '그중', '이중', '도중', '고등', '중고', '초중',
               '과정중', '수업중', '학습중', '재학중', '비중'}
    fallback = '고2 상담에서는 재학 중인 고등학교의 수업 자료와 현재 평가 범위를 준비해 주세요.'
        ...
        for group in reversed(groups):
            ...
            if high:
                ...
                if (not re.search(r'학교.{0,8}(?:정보|명|목록)|학교명', sentence)
                        ...
                        or re.search(r'\d|[A-Za-z]|[×÷=]', sentence.replace('고2', ''))):
                    continue
...
def replay_changes(raw_records, changes, check):
    ...
    for change in changes:
        ...
        check(change.get('grade') == '고2' and change.get('sourceMember') == source['sourceMember'], 'editorial-grade-source-identity', str(key))
        ...
        if change.get('stage') == 'grade-editorial':
            ...
            elif rule == 'gyoha-concealed-understanding' and key == ('교하', '영어'):
                expected = current
                for old, new in GYOHA:
        ...
        with ZipFile(path) as archive:
            ...
            for info in members:
                ...
                safe = (not member.is_absolute() and len(member.parts) == 2 and '..' not in member.parts
                        and member.parts[0] == '고2 ' + subject + '학원' and member.suffix == '.txt'
                ...
                records[key] = {'title': title, 'locality': locality, 'subject': subject, 'grade': '고2',
            ...
            facts.append({'path': path.relative_to(ROOT).as_posix(), 'sha256': archive_sha, 'members': len(members)})
...
def restore_existing_html(before, after, kind, child=None):
    """Reverse exact navigation-only changes; never discard content sections."""
    old_soup, soup = BeautifulSoup(before, 'html.parser'), BeautifulSoup(after, 'html.parser')
    current_graph, old_graph = graph(soup), graph(old_soup)
    expected_graph = copy.deepcopy(old_graph)
    canonical = old_soup.select_one('link[rel="canonical"]')['href']
    if kind == 'center':
        old_panel, new_panel = section_match(before, 'neighborhood-pages'), section_match(after, 'neighborhood-pages')
        if old_panel[0] != new_panel[0]:
            raise ValueError('Relocated center navigation markup changed')
        node = soup.select_one('#neighborhood-pages')
        if node.find_previous_sibling() != soup.select_one('#questions'):
            raise ValueError('Center navigation must immediately follow FAQ')
        addition = '<a href="#neighborhood-pages">동네별 학습 안내</a>'
        if after.count(addition) != 1 or not soup.select_one('.branch-toc a[href="#neighborhood-pages"]'):
            raise ValueError('Expected one added center TOC link')
        after = after[:new_panel.start()] + after[new_panel.end():]
        after = after.replace(addition, '', 1)
        before = before[:old_panel.start()] + before[old_panel.end():]
        for item in expected_graph:
            if item.get('@type') == 'WebPage':
                moved = {'@id': canonical + '#neighborhood-pages'}
                item['hasPart'].remove(moved)
                index = item['hasPart'].index({'@id': canonical + '#questions'}) + 1
                item['hasPart'].insert(index, moved)
    elif kind == 'neighborhood':
        match = section_match(after, 'child-pages')
        if match[0] != exact_child_panel(child):
            raise ValueError('Grade child panel differs from exact manifest-based contract')
        node = soup.select_one('#child-pages')
        if node.find_previous_sibling() != soup.select_one('#questions'):
            raise ValueError('Grade child navigation must immediately follow FAQ')
        addition = '<a href="#child-pages">학년별 학습 안내</a>'
        if after.count(addition) != 1 or not soup.select_one('#article-toc a[href="#child-pages"]'):
            raise ValueError('Expected one added neighborhood TOC link')
        after = after[:match.start()] + after[match.end():]
        after = after.replace(addition, '', 1)
        expected_graph.append({'@type': 'ItemList', '@id': canonical + '#child-pages', 'name': '학년별 학습 안내',
                               'numberOfItems': 1, 'itemListElement': [{'@type': 'ListItem', 'position': 1, 'name': child['title'], 'url': url(child['path'])}],
                               'isPartOf': {'@id': canonical + '#webpage'}})
        for item in expected_graph:
            if item.get('@type') == 'WebPage':
                index = item['hasPart'].index({'@id': canonical + '#questions'}) + 1
                item['hasPart'].insert(index, {'@id': canonical + '#child-pages'})
    else:
        raise ValueError('Unsupported existing-page category')
    # Node order has no JSON-LD meaning; fields and hasPart sequence do. No
    # change other than the explicit expected node/sequence above is allowed.
    if sorted(current_graph, key=lambda n: n['@id']) != sorted(expected_graph, key=lambda n: n['@id']):
        raise ValueError('Existing JSON-LD differs beyond the approved navigation relation')
...
def validate_new_page(record, source, display, parent, baseline, centers, reference, school_row, all_pages, check, counts, cache):
    ...
    scope_confirmed = '고2' in scoped_grades(center, ref, record['subject'])
    check(record['scopeConfirmed'] is scope_confirmed, 'manifest-current-high2-scope', path)
    ...
    check(record['path'] == parent['path'] + '고2/' and record['parentPath'] == parent['path']
    ...
    check(any(p.strong and p.strong.get_text() == '학습 대상' and p.get_text(' ', strip=True) == '학습 대상 고2 ' + record['subject'] for p in quick.select('p')), 'visible-exact-high2-learning-audience', path)
    ...
    if not scope_confirmed:
        expected_notice = '고2 ' + record['subject'] + ' 수업의 개설 여부와 상담 가능 학년을 먼저 확인해 주세요. 아래 글은 학습 준비를 위한 안내이며 해당 학년의 수업 개설을 뜻하지 않습니다.'
    ...
    check(article.get('@id') == canonical + '#article' and article.get('url') == canonical
          ...
          and article.get('description') == source['meta'] and article.get('educationalLevel') == '고2', 'exact-high2-article-identity', path)
    ...
    check(len(services) == int(scope_confirmed), 'service-only-exact-high2-confirmed', path)
    if services:
        service = services[0]
        check(service.get('@id') == canonical + '#service' and service.get('provider') == {'@id': center_canonical + '#center'}
              and service.get('serviceType') == '고2 ' + record['subject'] + ' 학습코칭'
...
def run(args):
    ...
    centers_with_children = {p['parentPath'].strip('/') + '/index.html' for p in parents}
    ...
    expected_sources = set(baseline['sourceFiles']) | {'tools/data/branch-grades/pages.json',
                        'tools/data/branch-grades/sources/고2 영어학원.zip', 'tools/data/branch-grades/sources/고2 수학학원.zip'}
    actual_sources = {p.relative_to(ROOT).as_posix() for p in (ROOT / 'tools/data').rglob('*') if p.is_file()}
    check(actual_sources == expected_sources, 'only-three-approved-new-source-artifacts')
    raw_records, archives = source_records(check)
    check({(r['grade'], r['subject'], r['sha256'], r['count']) for r in manifest['archives']}
          == {('고2', subject, digest, 371) for subject, digest in ARCHIVE_SHAS.items()}, 'manifest-original-archive-records')
    changes = read(REPORTS / '고2-editorial.json')['changes']
    ...
    check(counts['preservedCentersWithRelocatedNavigation'] == 188 and counts['preservedNeighborhoodParentsWithGradeLink'] == 742
          and counts['untouchedExistingPages'] == 2255, 'existing-page-scope-coverage')
    check(counts['newPages'] == 742 and counts['servicePages'] == 665 and counts['learningGuideOnlyPages'] == 77, 'grade-page-service-665-guide-77-coverage')
    generation = read(REPORTS / '고2-generation.json')
    check(generation['generatedPages'] == 742 and generation['scopeConfirmed'] == 665
    ...
    result = {'status': 'FAIL' if errors else 'PASS', 'grade': '고2', 'createdAt': datetime.now(timezone.utc).isoformat(),
...
def main():
    ...
    parser.add_argument('--report', type=Path, default=REPORTS / '고2-audit.json')
