"""Independent middle-school-years-1/2/3 source, navigation and preservation audit.
...
BASELINE = Path('C:/Users/1992k/Desktop/CodexData/tmp/site15-middle123-2026-09-10/before.json.gz')
DOMAIN = 'https://xn--9p4bn5e3wjn0a.com'
GRADES = ('중1', '중2', '중3')
ARCHIVE_SHAS = {
    ('중1', '영어'): 'baf422a465a25840e85f8482ff286843be821a69e5a705f59b5c3fbd6bbb127d',
    ('중1', '수학'): '38da0735ecc9304f05521dcee59fdae99aaab3cdc36bc98f521572dd60264558',
    ('중2', '영어'): '234c3852c02c12541bc349b4f828abe1967d21c348e2f7b6dc844c77b029f3e8',
    ('중2', '수학'): '8c6e1122c8f272db2965dbfaabe31c736677e446070c6d0a080b1ee81c0d55d4',
    ('중3', '영어'): 'e3d4c9d8f468e901112299c70bd5a87fa1bb6826ec17727b284784290b79813e',
    ('중3', '수학'): '6fa15a4edccb114a93a8069606598530fb519b1e61e4602cac5b8553610ddd39',
}

POLISHER_SHA256 = '6cf96be3eef5d91e97110468a2e3aeba8f443aeec90dc50c4d17b2c0a1597d2c'
GRADE_EDITOR_SHA256 = 'fcc27f238b9cbd92f2c6d677260f3b20d0a57ae9b78ed459e283b5de0972f3b9'
...
def reviewed_school_lists(value, grade):
    """Independently allow only clear current-student school enumerations.

    Progression, history, source cautions, and mixed location facts stay intact;
    surviving school names remain literal source substrings, never expanded.
    """
    ...
    history = re.compile(r'진학|출신|이전|과거|이력|초등학교 때|초등 시절|중학교 때|보조 자료|범위를 분리|상담과 범위|'
                         r'고등.{0,15}(?:준비|과정|이후)|고교.{0,15}(?:준비|과정|이후)|자료가 아니|확대해석|연결해 판단할 근거')
    fallback = grade + ' 상담에서는 재학 중인 중학교의 수업 자료와 현재 평가 범위를 준비해 주세요.'

    def sentence_update(sentence):
        if sentence.endswith('?') or not context.search(sentence) or history.search(sentence):
            return sentence
        ...
        for group in reversed(groups):
            kept = [m[1] for m in group if m[1].endswith(('중', '중학교'))]
            if len(kept) == len(group):
                continue
            if kept:
                tail = sentence[group[-1].end():]
                pairs = [('가 ', '이 '), ('는 ', '은 '), ('와 ', '과 '), ('를 ', '을 ')]
                if (ord(kept[-1][-1]) - 0xAC00) % 28 == 0:
                ...
                sentence = sentence[:group[0].start()] + ', '.join(kept) + tail
                  ...
                  and re.fullmatch(r'(?:제공된|안내된|참고) 학교 (?:정보|목록)(?:에는|에는|에) .+?'
...
def replay_changes(raw_records, changes, check):
    ...
    for change in changes:
        ...
        if change.get('stage') == 'grade-editorial':
            ...
            elif rule == 'middle-school-list-scope' and not field.endswith('.heading'):
                permitted = reviewed_school_lists(current, source['grade']) == change['after']
            elif rule == 'middle-reader-context' and not field.endswith('.heading'):
                expected = current
    ...
    for key, original in raw_records.items():
        ...
        check([s['heading'] for s in display['sections']] == [s['heading'] for s in original['sections']], 'all-middle-source-headings-unchanged', str(key))
...
def restore_existing_html(before, after, kind, children=None):
    """Reverse exactly three middle-grade links before the unchanged high1/high2 links."""
    ...
    if [r['grade'] for r in children] != list(GRADES):
        raise ValueError('Exactly middle1, middle2, middle3 children are required in order')
    ...
    if len(old_links) != 2 or not all(a['href'].endswith('/' + g + '/') for a, g in zip(old_links, ('고1', '고2'))):
        raise ValueError('Baseline must contain exactly the original high1/high2 links')
    ...
    if new_panel[0] != expected_panel:
        raise ValueError('Only exact middle1/2/3-before-high1/2 button additions are allowed')
    ...
    if len(listing) != 1 or listing[0].get('@type') != 'ItemList' or listing[0].get('numberOfItems') != 2:
        raise ValueError('Expected original two-child ItemList')
    if [n['position'] for n in listing[0]['itemListElement']] != [1, 2]:
        raise ValueError('Expected original high1/high2 ItemList positions')
    listing[0]['numberOfItems'] = 5
    ...
    if sorted(graph(soup), key=lambda n: n['@id']) != sorted(expected_graph, key=lambda n: n['@id']):
        raise ValueError('Schema changed outside exact middle1/2/3 ItemList additions')
...
def middle_school_names(row):
    names = []
    for raw in row.get('targetSchools', {}).get('middle', []):
...
def validate_new_page(record, source, display, parent, baseline, centers, reference, school_row, all_pages, check, counts, cache):
    ...
    check(record['scopeConfirmed'] is scope_confirmed, 'manifest-current-exact-middle-grade-scope', path)
    ...
    check(any(p.strong and p.strong.get_text() == '학습 대상' and p.get_text(' ', strip=True) == '학습 대상 ' + record['grade'] + ' ' + record['subject'] for p in quick.select('p')), 'visible-exact-middle-grade-learning-audience', path)
    ...
    check(all(g.summary.get_text() == '중학교' for g in groups), 'middle-school-level-only', path)
    check([n.get_text() for n in soup.select('#schools .branch-school-list li')] == middle_school_names(school_row), 'actual-reviewed-middle-school-names', path)
    ...
    check(article.get('@id') == canonical + '#article' and article.get('url') == canonical
          ...
          and article.get('description') == source['meta'] and article.get('educationalLevel') == record['grade'], 'exact-middle-grade-article-identity', path)
    ...
    check(len(services) == int(scope_confirmed), 'service-only-exact-middle-grade-confirmed', path)
    if services:
        service = services[0]
...
def run(args):
    ...
    check(len(baseline['pages']) + len(extras['pages']) == len(extras['sitemapUrls']) == 4672,
          'pre-middle-checkpoint-covers-all-4672-public-pages')
    ...
    check(len(pages) == 2226 and len(children_by_parent) == 742
          and all([r['grade'] for r in rows] == list(GRADES) for rows in children_by_parent.values()),
          '2226-new-middle-pages-three-per-742-parents')
    ...
    check(sha(prior_manifest_bytes) == baseline['sourceFiles']['tools/data/branch-grades/pages.json']['sha256'],
          'merged-manifest-reverses-to-exact-original-high1-high2-manifest')
    check(len(manifest['pages']) == 3710 and {r['grade'] for r in manifest['pages']} == set(GRADES) | {'고1', '고2'},
          'manifest-only-original-high-grades-and-three-new-middle-grades')
    new_paths = {r['path'].strip('/') + '/index.html' for r in pages}
    check(len(new_paths) == 2226 and not new_paths.intersection(baseline['pages']), 'new-routes-unique-and-no-existing-overwrite')
    ...
    check(actual == expected, 'complete-path-set-exactly-2226-additions')
    for relative, old in baseline['pages'].items():
        ...
        if relative in children_by_parent:
            try:
                ...
                counts['newParentGradeButtons'] += 3
    ...
    check(actual_sources == expected_sources, 'only-six-approved-new-archive-artifacts')
    ...
    check({(r['grade'], r['subject'], r['sha256'], r['count']) for r in manifest['archives'] if r['grade'] in GRADES}
          ...
          'manifest-six-original-archive-records')
    ...
    check(set(raw_records) == expected_keys == {(p['locality'], p['subject'], p['grade']) for p in pages},
          'exact-2226-source-parent-child-key-coverage')
    ...
    check(counts['preservedNeighborhoodParentsWithGradeLinks'] == 742 and counts['untouchedExistingPages'] == 3927,
          'existing-page-scope-coverage')
    check(counts['newPages'] == 2226 and counts['servicePages'] == 2172 and counts['learningGuideOnlyPages'] == 54,
          'grade-page-service-2172-guide-54-coverage')
    for grade in GRADES:
        generation = read(REPORTS / (grade + '-generation.json'))
        grade_pages = [p for p in pages if p['grade'] == grade]
        check(len(grade_pages) == generation['generatedPages'] == 742 and generation['scopeConfirmed'] == 724
        ...
        for subject, confirmed in (('영어', 363), ('수학', 361)):
    ...
    check(counts['preservedHigh1Pages'] == counts['preservedHigh2Pages'] == 742,
          'every-existing-high1-high2-html-byte-preserved')
    ...
    check(set(locs) == set(extras['sitemapUrls']) | {url(r['path']) for r in pages} and len(locs) == 6898,
          'sitemap-all-original-4672-plus-only-new-2226')
...
def main():
    ...
    parser.add_argument('--report', type=Path, default=REPORTS / '중1-중3-audit.json')
