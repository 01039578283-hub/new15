"""Independent elementary-school-years-5/6 source, navigation and preservation audit.
...
BASELINE = Path('C:/Users/1992k/Desktop/CodexData/tmp/site15-elementary56-2026-09-10/before.json.gz')
DOMAIN = 'https://xn--9p4bn5e3wjn0a.com'
GRADES = ('초5', '초6')
EXISTING_GRADES = ('중1', '중2', '중3', '고1', '고2')
ARCHIVE_SHAS = {
    ('초5', '영어'): 'c43cfb1033a85a94288c6298db29aa5ee5c0d47da3f25eddc1e4050974306b89',
    ('초5', '수학'): 'fba1406e2eabc5bc99aff384bee15967a8ff84e10d655c0ba2681c3acc8870f9',
    ('초6', '영어'): '884a6a57b0a982dc1c45712656d10de103dfdd0c1d84a87f14f70c0536af7aa7',
    ('초6', '수학'): '0cef3cf7e5c5d8eebc47864b9e7c3d731af044ce659a7316987e7af132895953',
}
SCOPE_CONFIRMED = {('초5', '영어'): 357, ('초5', '수학'): 355, ('초6', '영어'): 363, ('초6', '수학'): 361}

POLISHER_SHA256 = '6cf96be3eef5d91e97110468a2e3aeba8f443aeec90dc50c4d17b2c0a1597d2c'
GRADE_EDITOR_SHA256 = '88003890ee9060e73363865b42a30fd036b6ef38220c2073a3dc659ec98c4840'
...
WRITING_NOTES = re.compile(r'원고\s*참고\s*키워드|[FG]\s*(?:열)?에\s*제시된|입력된\s*학교\s*정보|(?<![가-힣A-Za-z0-9_])이\s*원고|상담 참고 항목로')
...
def complete_sentence_selection(before, after, grade):
    ...
    for i, sentence in enumerate(old):
        if i in selected:
            continue
        if re.search(r'\d|[A-Za-z]|[×÷=]|[‘“「]|다만|단정|보장|개설|운영 여부', sentence.replace(grade, '')):
            return False
...
def reviewed_school_lists(value, grade):
    """Permit only literal elementary names in clear current-student lists.

    Any progression/history context protects the whole original paragraph.
    No school alias, inferred school, or new operating fact may be introduced.
    """
    school = re.compile(r'(?<![가-힣A-Za-z0-9])([가-힣0-9]{1,16}?(?:초등학교|중학교|고등학교|여중|여고|초|중|고))'
                        r'(?=(?:이나|나|처럼|에서|은|는|이|가|을|를|와|과|로|의|,|·|/|\s|$))')
    ignored = {'참고', '그리고', '말고', '않고', '보고', '집중', '그중', '이중', '도중', '고등', '중고', '초중',
               '과정중', '수업중', '학습중', '재학중', '비중'}
    context = re.compile(r'(?:제공된|제시된|안내된|참고|입력된).{0,12}학교|학교.{0,30}(?:제공|제시|포함)|학교명이 상담 대상|학교 가운데')
    history = re.compile(r'진학|출신|이전|과거|이력|보조 자료|범위를 분리|상담과 범위|'
                         r'중(?:학교|등|학).{0,20}(?:준비|과정|이후|연결|전환|대비)|'
                         r'고(?:등학교|등|교).{0,20}(?:준비|과정|이후|연결|전환|대비)|'
                         r'자료가 아니|확대해석|연결해 판단할 근거')
    if history.search(value):
        return value
    fallback = grade + ' 상담에서는 재학 중인 초등학교의 수업 자료와 현재 학습 범위를 준비해 주세요.'

    def sentence_update(sentence):
        if sentence.endswith('?') or not context.search(sentence):
            return sentence
        ...
        for group in reversed(groups):
            kept = [m[1] for m in group if m[1].endswith(('초', '초등학교'))]
            ...
            if kept:
                ...
                for old, new in [('이 ', '가 '), ('은 ', '는 '), ('과 ', '와 '), ('을 ', '를 ')]:
                    if tail.startswith(old):
                        ...
                        break
                sentence = sentence[:group[0].start()] + ', '.join(kept) + tail
            elif (not any(m[1].endswith(('초', '초등학교')) for m in names)
                  and not re.search(r'\d|[A-Za-z]|[×÷=]', sentence.replace(grade, ''))
                  and not re.search(r'지역 정보|이 학교들|이 이름들|그 학교|이것만으로', value)
                  and re.fullmatch(r'(?:제공된|안내된|참고) 학교 (?:정보|목록)(?:에는|에) .+?'
                                   r'(?:포함되어 있습니다|제시되어 있습니다|있습니다)\.', sentence)):
                return fallback
        return sentence

    return re.sub(r'[^.!?]+[.!?](?:[”’])?|[^.!?]+$',
                  lambda m: m[0][:len(m[0]) - len(m[0].lstrip())] + sentence_update(m[0].lstrip()), value)
...
def replay_changes(raw_records, changes, check):
    ...
    middle_phrases = literal_table(grade_path, '_MIDDLE_READER_PHRASES')
    middle_function = next(n for n in grade_ast.body if isinstance(n, ast.FunctionDef) and n.name == '_middle_reader_context')
    middle_starts = next(ast.literal_eval(n.value) for n in middle_function.body if isinstance(n, ast.Assign)
                        and any(isinstance(t, ast.Name) and t.id == 'starts' for t in n.targets))
    elementary_phrases = literal_table(grade_path, '_ELEMENTARY_READER_PHRASES')
    records, counts = copy.deepcopy(raw_records), Counter()
    for change in changes:
        key = (change['locality'], change['subject'], change['grade'])
        field = change['field']
        ...
        check(change.get('grade') in GRADES and change.get('sourceMember') == source['sourceMember'], 'editorial-grade-source-identity', str(key))
        ...
        if change.get('stage') == 'grade-editorial':
            ...
            if rule == 'grade-complete-sentence-intro' and field == 'intro':
                permitted = complete_sentence_selection(current, change['after'], source['grade'])
            ...
            elif rule == 'grade-source-copy-context' and key[:2] == ('다산신도시', '수학') and re.fullmatch(r'sections\[\d+\]\.paragraphs\[\d+\]', field):
                permitted = current.replace('이 원고의 제공 사실에 포함되어 있지 않으므로', '이 안내에서 확인되지 않았으므로') == change['after']
            elif rule == 'elementary-school-list-scope' and not field.endswith('.heading'):
                permitted = reviewed_school_lists(current, source['grade']) == change['after']
            elif rule == 'elementary-reader-context' and not field.endswith('.heading'):
                expected = current
                ...
                for old, new in middle_phrases:
                    expected = expected.replace(old, new)
                for old, new in middle_starts:
                    expected = re.sub(r'(\A|(?<=[.!?])\s+)' + re.escape(old), lambda m: m[1] + new, expected)
                for old, new in elementary_phrases:
                    expected = expected.replace(old, new)
    ...
    for key, original in raw_records.items():
        ...
        check([s['heading'] for s in display['sections']] == [s['heading'] for s in original['sections']], 'all-elementary-source-headings-unchanged', str(key))
...
def source_records(check, source_dir=None):
    ...
    records, facts = {}, []
    for grade, subject in ARCHIVE_SHAS:
        path = (Path(source_dir) if source_dir else DATA / 'sources') / (grade + ' ' + subject + '학원.zip')
        archive_sha = sha(path.read_bytes())
        check(archive_sha == ARCHIVE_SHAS[(grade, subject)], 'original-preflight-archive-sha256', path.name)
        ...
        with ZipFile(path) as archive:
            ...
            for info in members:
                ...
                safe = (not member.is_absolute() and len(member.parts) == 2 and '..' not in member.parts
                        and member.parts[0] == grade + ' ' + subject + '학원' and member.suffix == '.txt'
                ...
                key = (locality, subject, grade)
                check(key not in records, 'source-unique-key', name)
                records[key] = {'title': title, 'locality': locality, 'subject': subject, 'grade': grade,
...
def exact_child_panel(children):
...
def restore_existing_html(before, after, kind, children=None):
    """Reverse exactly two elementary links before the unchanged five grade links."""
    if kind != 'neighborhood' or children is None:
        raise ValueError('Only existing neighborhood parent navigation may change')
    if [r['grade'] for r in children] != list(GRADES):
        raise ValueError('Exactly elementary5/6 children are required in order')
    ...
    if len(old_links) != 5 or not all(a['href'].endswith('/' + g + '/') for a, g in zip(old_links, EXISTING_GRADES)):
        raise ValueError('Baseline must contain exactly the original five middle/high grade links')
    link = ''.join('<a class="branch-child-link" href="' + escape(r['path'], quote=True) + '">'
                   + escape(r['title']) + '</a>' for r in children)
    ...
    if new_panel[0] != expected_panel:
        raise ValueError('Only exact elementary5/6-before-existing-five-grade button additions are allowed')
    ...
    if len(listing) != 1 or listing[0].get('@type') != 'ItemList' or listing[0].get('numberOfItems') != 5:
        raise ValueError('Expected original five-child ItemList')
    if [n['position'] for n in listing[0]['itemListElement']] != [1, 2, 3, 4, 5]:
        raise ValueError('Expected original five middle/high grade ItemList positions')
    listing[0]['numberOfItems'] = 7
    for old in listing[0]['itemListElement']:
        old['position'] += 2
    listing[0]['itemListElement'] = [
        {'@type': 'ListItem', 'position': i, 'name': r['title'], 'url': url(r['path'])}
        for i, r in enumerate(children, 1)] + listing[0]['itemListElement']
    if sorted(graph(soup), key=lambda n: n['@id']) != sorted(expected_graph, key=lambda n: n['@id']):
        raise ValueError('Schema changed outside exact elementary5/6 ItemList additions')
...
def elementary_school_names(row):
    names = []
    for raw in row.get('targetSchools', {}).get('elementary', []):
        value = re.sub(r'\[([^\]]*(?:후보|확인\s*필요|미확인|\d{4}\s*통계)[^\]]*)\]', '', raw).strip()
        if re.sub(r'\s+', '', value) in ('지역내모든고등학교가능', '모든고등학교가능', '특성화고', '기타사립초'):
            continue
...
def validate_new_page(record, source, display, parent, baseline, centers, reference, school_row, all_pages, check, counts, cache):
    ...
    scope_confirmed = record['grade'] in scoped_grades(center, ref, record['subject'])
    check(record['scopeConfirmed'] is scope_confirmed, 'manifest-current-exact-elementary-grade-scope', path)
    ...
    check(record['path'] == parent['path'] + record['grade'] + '/' and record['parentPath'] == parent['path']
    ...
    check(any(p.strong and p.strong.get_text() == '학습 대상' and p.get_text(' ', strip=True) == '학습 대상 ' + record['grade'] + ' ' + record['subject'] for p in quick.select('p')), 'visible-exact-elementary-grade-learning-audience', path)
    ...
    if not scope_confirmed:
        expected_notice = record['grade'] + ' ' + record['subject'] + ' 수업의 개설 여부와 상담 가능 학년을 먼저 확인해 주세요. 아래 글은 학습 준비를 위한 안내이며 해당 학년의 수업 개설을 뜻하지 않습니다.'
    ...
    check(all(g.summary.get_text() == '초등학교' for g in groups), 'elementary-school-level-only', path)
    check([n.get_text() for n in soup.select('#schools .branch-school-list li')] == elementary_school_names(school_row), 'actual-reviewed-elementary-school-names', path)
    ...
    expected_related = sorted([r for r in all_pages if r['centerId'] == record['centerId'] and r['grade'] == record['grade'] and r['path'] != path],
    ...
    check(article.get('@id') == canonical + '#article' and article.get('url') == canonical
          ...
          and article.get('description') == source['meta'] and article.get('educationalLevel') == record['grade'], 'exact-elementary-grade-article-identity', path)
    ...
    check(len(services) == int(scope_confirmed), 'service-only-exact-elementary-grade-confirmed', path)
    if services:
        service = services[0]
        check(service.get('@id') == canonical + '#service' and service.get('provider') == {'@id': center_canonical + '#center'}
              and service.get('serviceType') == record['grade'] + ' ' + record['subject'] + ' 학습코칭'
    ...
    check(WRITING_NOTES.search(visible) is None,
...
def run(args):
    ...
    check(len(baseline['pages']) + len(extras['pages']) == len(extras['sitemapUrls']) == 6898,
          'pre-elementary-checkpoint-covers-all-6898-public-pages')
    pages = [r for r in manifest['pages'] if r['grade'] in GRADES]
    ...
    for children in children_by_parent.values():
        children.sort(key=lambda r: r['grade'])
    check(len(pages) == 1484 and len(children_by_parent) == 742
          and all([r['grade'] for r in rows] == list(GRADES) for rows in children_by_parent.values()),
          '1484-new-elementary-pages-two-per-742-parents')
    ...
    prior_manifest['archives'] = [r for r in manifest['archives'] if r['grade'] not in GRADES]
    prior_manifest['pages'] = [r for r in manifest['pages'] if r['grade'] not in GRADES]
    ...
    check(sha(prior_manifest_bytes) == baseline['sourceFiles']['tools/data/branch-grades/pages.json']['sha256'],
          'merged-manifest-reverses-to-exact-original-five-grade-manifest')
    check(len(manifest['pages']) == 5194 and {r['grade'] for r in manifest['pages']} == set(GRADES) | set(EXISTING_GRADES),
          'manifest-only-original-five-grades-and-two-new-elementary-grades')
    new_paths = {r['path'].strip('/') + '/index.html' for r in pages}
    check(len(new_paths) == 1484 and not new_paths.intersection(baseline['pages']), 'new-routes-unique-and-no-existing-overwrite')
    expected = set(baseline['pages']) | new_paths
    actual = {p.relative_to(ROOT).as_posix() for top in ('지점안내', '과목별학원') for p in (ROOT / top).rglob('index.html')}
    check(actual == expected, 'complete-path-set-exactly-1484-additions')
    for relative, old in baseline['pages'].items():
        raw = (ROOT / relative).read_bytes()
        if relative in children_by_parent:
            try:
                restore_existing_html(old['html'], raw.decode('utf-8'), 'neighborhood', children_by_parent[relative])
                counts['preservedNeighborhoodParentsWithGradeLinks'] += 1
                counts['newParentGradeButtons'] += 2
            ...
            if relative.endswith('/고1/index.html'):
                counts['preservedHigh1Pages'] += 1
            elif relative.endswith('/고2/index.html'):
                counts['preservedHigh2Pages'] += 1
            elif any(relative.endswith('/' + g + '/index.html') for g in ('중1', '중2', '중3')):
                counts['preservedMiddlePages'] += 1
    for category in ('assets', 'sourceFiles'):
        for relative, old in baseline[category].items():
            if relative == 'tools/data/branch-grades/pages.json':
                continue  # Independently reversed and fingerprint-checked above.
            path = ROOT / relative
    ...
    expected_sources = set(baseline['sourceFiles']) | {
        'tools/data/branch-grades/sources/' + grade + ' ' + subject + '학원.zip'
        for grade, subject in ARCHIVE_SHAS}
    actual_sources = {p.relative_to(ROOT).as_posix() for p in (ROOT / 'tools/data').rglob('*') if p.is_file()}
    check(actual_sources == expected_sources, 'only-four-approved-new-archive-artifacts')
    raw_records, archives = source_records(check)
    check({(r['grade'], r['subject'], r['sha256'], r['count']) for r in manifest['archives'] if r['grade'] in GRADES}
          == {(grade, subject, digest, 371) for (grade, subject), digest in ARCHIVE_SHAS.items()},
          'manifest-four-original-archive-records')
    changes = [change for grade in GRADES for change in read(REPORTS / (grade + '-editorial.json'))['changes']]
    ...
    expected_keys = {(locality, subject, grade) for locality, subject in parent_by_key for grade in GRADES}
    check(set(raw_records) == expected_keys == {(p['locality'], p['subject'], p['grade']) for p in pages},
          'exact-1484-source-parent-child-key-coverage')
    cache, scope_counts = {}, Counter()
    for record in pages:
        try:
            key = (record['locality'], record['subject'], record['grade'])
            validate_new_page(record, raw_records[key], displays[key], parent_by_key[key[:2]], baseline, centers,
                              reference, schools.get(record['locality'], {}), pages, check, counts, cache)
            actual_scope = record['grade'] in scoped_grades(centers[record['centerId']],
                reference['centers'].get(record['centerId'], {}), record['subject'])
            scope_counts[(record['grade'], record['subject'], actual_scope)] += 1
    ...
    check(counts['preservedNeighborhoodParentsWithGradeLinks'] == 742 and counts['untouchedExistingPages'] == 6153,
          'existing-page-scope-coverage')
    check(counts['newPages'] == 1484 and counts['servicePages'] == 1436 and counts['learningGuideOnlyPages'] == 48,
          'grade-page-service-1436-guide-48-coverage')
    for grade in GRADES:
        generation = read(REPORTS / (grade + '-generation.json'))
        grade_pages = [p for p in pages if p['grade'] == grade]
        check(len(grade_pages) == generation['generatedPages'] == 742 and generation['scopeConfirmed'] == sum(SCOPE_CONFIRMED[(grade, subject)] for subject in ('영어', '수학'))
              and set(generation['scopeConfirmationNeeded']) == {p['path'] for p in grade_pages if not p['scopeConfirmed']},
              'generation-report-matches-audited-scope', grade)
        check(generation['editorialChanges'] == len([c for c in changes if c['grade'] == grade]),
              'generation-editorial-log-count', grade)
        for subject in ('영어', '수학'):
            confirmed = SCOPE_CONFIRMED[(grade, subject)]
            check(scope_counts[(grade, subject, True)] == confirmed and scope_counts[(grade, subject, False)] == 371 - confirmed,
                  'exact-grade-subject-scope-count', grade + ' ' + subject)
    check(counts['preservedHigh1Pages'] == counts['preservedHigh2Pages'] == 742 and counts['preservedMiddlePages'] == 2226,
          'every-existing-3710-middle-high-grade-html-byte-preserved')
    ...
    check(set(locs) == set(extras['sitemapUrls']) | {url(r['path']) for r in pages} and len(locs) == 8382,
          'sitemap-all-original-6898-plus-only-new-1484')
    counts['publicSitemapPages'] = len(locs)
    check(len(ET.parse(ROOT / 'rss.xml').findall('.//item')) == 9, 'rss-valid-nine-items')
    sitemap_counts = Counter(locs)
    for record in pages:
        check(sitemap_counts[url(record['path'])] == 1, 'grade-sitemap-exactly-once', record['path'])
    result = {'status': 'FAIL' if errors else 'PASS', 'grades': list(GRADES), 'createdAt': datetime.now(timezone.utc).isoformat(),
              'baseline': str(args.baseline), 'counts': dict(counts), 'archives': archives, 'editorialCounts': editorial_counts,
              'scopeCounts': [{'grade': g, 'subject': s, 'confirmed': yes, 'pages': count}
                              for (g, s, yes), count in sorted(scope_counts.items())],
              'errors': errors, 'visualRender': 'Not claimed; browser interaction checks are performed separately by root.'}
...
def main():
    ...
    parser.add_argument('--report', type=Path, default=REPORTS / '초5-초6-audit.json')
